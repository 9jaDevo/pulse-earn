/*
  # Ambassador Commission Tiers and Payout System

  1. New Tables
    - `ambassador_commission_tiers` - Stores tier-based commission rates
  
  2. Functions
    - `calculate_ambassador_commission` - Calculates commission rate based on referrals
    - `process_referral_commission` - Processes commission for referrals
    - `get_ambassador_tier` - Gets ambassador's current tier information
  
  3. Triggers
    - Updates ambassador commission rate when referral count changes
*/
...
-- Create RLS policies (with checks to avoid duplicate policy errors)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'ambassador_commission_tiers' 
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'ambassador_commission_tiers' 
    AND policyname = 'Users can view commission tiers'
  ) THEN
...
BEGIN
  IF EXISTS (
    ...
    WHERE tgname = 'update_ambassador_commission_rate_trigger'
  ) THEN
    DROP TRIGGER update_ambassador_commission_rate_trigger ON ambassadors;
  END IF;
END
$$;
